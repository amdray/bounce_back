...
| Действие при входе | **PARTIAL** | C: считает `time_bonus`, добавляет stage bonus, переходит на следующий уровень через `reload_level_state()`. Отличие: на финальном уровне (`level_index == 20`) выполняется завершение приложения (`running = 0`) вместо отдельной win-сцены/flow из MIDP. |
