...
| **Коэффициент затухания (плоская поверхность)** | **MATCH** | Java: `G >>= 1` (50%). C: `p->bounce_state >>= 1` — исправлено в текущем `player.c` (строки 888, 960, 1279, 1456). |
| Коэффициент затухания (склоны) | **MATCH** | Java: `G = 3*G >> 2` (75%). C: `bounce_state = (3 * bounce_state) >> 2` (`player.c:1023`). |

> **RESOLVED 4.5 — Затухание отскока на плоской поверхности исправлено**  
> **Факт:** `player.c` содержит `p->bounce_state >>= 1` (50%, деление на 2) в строках 888, 960, 1279, 1456 — flat surface cases. Склоны используют `(3 * p->bounce_state) >> 2` (75%) — соответствует Java. Поведение идентично оригиналу.
...
|---|---|---|---|
| 1 | ~~MISMATCH 4.5~~ **RESOLVED** | Bounce decay исправлен: `>>= 1` (50%) в `player.c` строки 888, 960, 1279, 1456 | — |
