...
**Результат:** `release/EBOOT.PBP` - **технодемо** базового движка (движение по уровню, нет врагов/бонусов)

### Отсутствует (не реализовано)

❌ **Геймплей:**
- Враги (полностью отсутствуют)
- Бонусы и коллекционные объекты
- Специальные тайлы (батуты, телепорты, движущиеся платформы)
- Смерть/респавн игрока
- Условия победы (выход из уровня)
- UI и HUD (очки, жизни, таймер)
