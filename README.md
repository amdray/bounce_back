...
> Примечание: отчёт отражает состояние на момент проведения аудита. Часть пунктов уже закрыта в текущем `src/` и трекается в статусе ниже.

**Ключевые выводы:**
- **1 критическое расхождение**: трансформации тайлов не применяются в основном проходе (`level_renderer.c`). ~~Bounce decay 75% vs 50%~~ — исправлено.
- **5 значительных расхождений**: отсутствие рендера двери, неправильный alpha водных тайлов, захардкоженный цвет фона, и др. (см. полный список в аудите).
...
- ✅ **Game Over обработка** (`main.c`) - при 0 жизней сброс на уровень 0 с восстановлением жизней
...
- ❌ Экран Game Over / Win как отдельные UI-сцены (сейчас есть только логика переходов)
- ❌ Меню, пауза и полноценная навигация как в оригинальном UI-потоке
- ❌ Звук и музыка (SFX/BGM)
- ❌ 60 FPS render loop при сохранении fixed 20 Hz simulation (сейчас общий цикл 20 FPS)

**Дополнительно:**
- Применение tile transform в основном pass рендера (`level_renderer.c` пока без `draw_tile_with_transform`)
- Финальная сверка визуала с original по alpha/цветам отдельных тайлов и слоёв
- Рендер спрайта выходной двери (`exit_door.c` — логика есть, спрайт отсутствует)
...
- ✅ Камера и скроллинг
- ✅ Ввод (PSP controller / SDL gamecontroller)
- ✅ HUD (очки, жизни, кольца, бонусы)
...
- ✅ Рендер врагов (`enemy_renderer`)
- ✅ Выход уровня и прогрессия (`exit_door` + переход на next level)
- ✅ Обработка Game Over (сброс состояния и рестарт с level 0)
- ✅ Таймер уровня для bonus score (`SDL_GetTicks`)

### Отсутствует / Требует исправлений
- ❌ Отдельные экраны Game Over/Win и UI-flow меню
- ❌ Звук и музыка
- ❌ Основной level pass пока не применяет `meta.transform` (transform есть в foreground pass)
- ❌ Правильный alpha для водных тайлов (непрозрачность)
- ❌ Правильный цвет фона (захардкожен, адаптация для PSP)
- ❌ Разделение simulation/render loops (fixed 20 Hz + render 60 Hz)
